...
import { cartInvitation, getCartDetails, removeCartMember} from "../../../actions/cartActions";
import { clearErrors } from "../../../actions/cartActions";
import { CART_INVITATION_RESET, REMOVE_CART_MEMBER_RESET } from "../../../constants/cartConstants";
...
export default function Members({id}) {
    ...
    const {error,loading,isInvited,isRemoved} = useSelector((state)=>state.invitation);
    ...
    useEffect(()=>{
        ...
        if (isInvited === true) {
          alert.success("Invitation Send Successfully");
          dispatch({
            type: CART_INVITATION_RESET,
          });
        ...
        if (isRemoved === true) {
          ...
          dispatch({
            type: REMOVE_CART_MEMBER_RESET,
          });

          dispatch(getCartDetails());
        }
    },[dispatch,error,isInvited,alert,isRemoved]);
  ...
  return (
    <div className="adduserdiv">
      ...
      {cartDetails && cartDetails.members && cartDetails.members.length > 0 ? (
        cartDetails.members.filter((mem)=>mem.user._id !== user?._id).map((member, index) => (
            ...
            <div style={{ textTransform: "capitalize" }}>
               {member.status === 'canceled' ? (
                <>
                 ...
                 <MdDeleteOutline onClick={(e) => removeMember(e, member?.user)} style={{ cursor: "pointer" }}/>
                </>
